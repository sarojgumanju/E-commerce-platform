<x-frontend-layout>
    ...
    <div class="py-8 lg:py-12">
        <div class="container mx-auto px-4" style="width: 86%; max-width: 1400px;">
            ...
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="text-3xl lg:text-4xl font-bold text-[#0b1e33] mb-2">Order History</h1>
                <div class="decorative-line"></div>
                <p class="text-[#475569] mt-2">Track and manage your orders</p>
            </div>
            ...
            @if(!$orders || count($orders) == 0)
                ...
                <div class="bg-white rounded-2xl shadow-card border border-gray-100 text-center py-16 px-4">
                    ...
                    <h3 class="text-2xl font-semibold text-[#0f172a] mb-3">No orders yet!</h3>
                    <p class="text-[#475569] mb-8">You haven't placed any orders yet. Start shopping to see your orders here.</p>
                                ...
                                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                    <div>
                                        <div class="flex items-center gap-3 mb-2">
                                            <h4 class="text-xl font-bold text-[#0b1e33]">Order #{{ $order->id }}</h4>
                                    ...
                                    <div class="text-right">
                                        <div class="text-sm text-[#475569] mb-1">Total Amount</div>
                                        <div class="text-2xl font-bold text-[#f97316]">Rs. {{ number_format($order->total_amount, 2) }}</div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="p-5 hover:bg-[#fafafa] transition-colors">
                                        <div class="flex flex-col md:flex-row gap-4 items-start md:items-center">
                                            ...
                                            <div class="md:w-32 text-center">
                                                <div class="text-sm text-[#475569]">Quantity</div>
                                                <div class="font-bold text-[#0f172a] text-lg">{{ $item->qty }}</div>
                                            </div>
                                ...
                                <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                                    <div class="flex gap-4">
                                        @if($order->payment_status == 'Pending')
                                            <a href="{{ route('khalti.initiate', $order->id) }}" class="btn-primary inline-flex text-sm px-6 py-2">
                                                <i class="fas fa-credit-card mr-2"></i> Complete Payment
                                            </a>
                                        @endif
                                        <button onclick="window.print()" class="text-[#0b1e33] border-2 border-gray-300 rounded-lg font-medium hover:bg-gray-50 transition px-4 py-2">
                                            <i class="fas fa-print mr-2"></i> Print Order
                                        </button>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-xs text-[#475569]">Order Total</div>
                                        <div class="text-xl font-bold text-[#0b1e33]">Rs. {{ number_format($order->total_amount, 2) }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <style>
        ...
        @media print {
            .navbar, .footer, .btn-primary, button {
